...
export default function IdentityPage() {
  const { identity: convexIdentity, saveIdentity, loading } = useSettings();
  const [identity, setIdentity] = useState<InfluencerIdentity | null>(null);
  const [showResetConfirm, setShowResetConfirm] = useState(false);
  const [hasChanges, setHasChanges] = useState(false);

  // Populate local state once Convex data arrives
  useEffect(() => {
    if (!loading && convexIdentity && !identity) {
      setIdentity(convexIdentity);
    }
  }, [loading, convexIdentity, identity]);
  ...
  const handleReset = async () => {
    ...
    await saveIdentity(DEFAULT_IDENTITY);
    setHasChanges(false);
